...
async function recalcularStats(pool, empresaId) {
  ...
  // Imágenes que ya viven en R2 (tienen url) — no ocupan espacio real en Neon
  const enR2 = await pool.query(
    'SELECT COUNT(*) as cantidad, COALESCE(SUM(size_bytes), 0) as bytes FROM images WHERE empresa_id = $1 AND url IS NOT NULL',
    [empresaId]
  );
  // Imágenes viejas que todavía están en base64 dentro de Postgres (antes de migrar a R2)
  const legadoEnDB = await pool.query(
    'SELECT COUNT(*) as cantidad, COALESCE(SUM(LENGTH(data)), 0) as bytes FROM images WHERE empresa_id = $1 AND url IS NULL AND data IS NOT NULL',
    [empresaId]
  );
  ...
  const bytesDbEstimados = Number(textoBytes.rows[0].bytes) + Number(legadoEnDB.rows[0].bytes);
  const bytesR2 = Number(enR2.rows[0].bytes);
  const cantidadImagenes = Number(enR2.rows[0].cantidad) + Number(legadoEnDB.rows[0].cantidad);
